    ...
    Alter.find()
        ...
        .then(async alters => {
            ...
            for (const alter of alters) {
                Alter.findOne({ _id: alter._system_id })
                    .exec()
                    .then(alter => {
...
altersRouter.post('/', async (req, res, next) => {
    ...
    findUser(req.body.user_id).then(user => {
        ...
        System.findOne({ system_user_id: req.body.user_id })
            .exec()
            .then(system => {
                if (!system)
                    return res.status(404).json({
                ...
                for (const alter_id of system.alter_ids) {
                    ...
                    new Alter({
                        ...
                        .catch(async err => {
                            ...
                            const IS_USER_ID_CONFLICT =
                                err.code === 11000 && err.keyValue.system_user_id;
